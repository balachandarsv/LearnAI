...
const MEME_OR_NOT_MODEL_URL =
    chrome.extension.getURL('mobilenetv2/model.json')
...
class ImageClassifier {
  ...
  async loadModel() {
    ...
    try {
      this.model = await tf.loadLayersModel(MEME_OR_NOT_MODEL_URL);
      ...
      console.error(
          `Unable to load model from URL: ${MEME_OR_NOT_MODEL_URL}`);
    }
  }
  ...
  async predict(imgElement) {
    ...
    const logits = tf.tidy(() => {
      ...
      if (output.shape[output.shape.length - 1] === 1) {
        return output;
      } else {
